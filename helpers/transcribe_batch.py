...
NOTE: FunASR loads models once and shares them across workers via the
thread pool, so the first file is slow (model download + load) but
subsequent files are fast.
...
def main() -> None:
    ...
    print(f"transcribing {len(pending)} files with {args.workers} parallel worker(s)")
    print("  (FunASR models are shared across threads — first file includes model load)")
    t0 = time.time()
