...
# specific reason.
#
# Font: Noto Sans CJK SC (思源黑体) — bundled in static/fonts/ at repo root.
# Falls back to PingFang SC on macOS if not found.
SUB_FORCE_STYLE = (
...
# Resolve subtitle font file path for libass / ffmpeg subtitles filter.
# libass needs the actual font file for ASS/SSA subtitles.
import os as _os
_SKILL_DIR = Path(__file__).resolve().parent.parent
_FONT_CANDIDATES = [
    _SKILL_DIR / "static" / "fonts" / "NotoSansCJKsc-Bold.otf",
    _SKILL_DIR / "static" / "fonts" / "NotoSansCJKsc-Regular.otf",
...
# For draft mode: fast HDR→SDR using only colorspace (handles tone mapping internally)
DRAFT_TONEMAP = "colorspace=all=bt709:iall=bt2020:itrc=bt709:ispace=bt709:range=tv"

# Full quality: linear light tonemapping for precise HDR→SDR conversion
FULL_TONEMAP = (
    "format=gbrpf32le,"
    "tonemap=tonemap=hable:desat=0:peak=1000,"
    "colorspace=all=bt709:iall=bt2020:itrc=bt709:ispace=bt709:range=tv,format=yuv420p"
)
...
def extract_segment(
    ...
    if is_hdr_source(source):
        if draft:
            # Draft: skip expensive tone mapping; hardware decode/encode handles it
            pass
        else:
            tonemap = FULL_TONEMAP
    ...
    if draft:
        ...
        codec, codec_args = "h264_videotoolbox", ["-profile", "high"]
        hwaccel = ["-hwaccel", "videotoolbox"]
    elif preview:
        preset, crf = "medium", "22"
        codec, codec_args = "libx264", ["-preset", preset, "-crf", crf]
        hwaccel = []
    else:
        preset, crf = "fast", "20"
        codec, codec_args = "libx264", ["-preset", preset, "-crf", crf]
...
def build_master_srt(edl: dict, edit_dir: Path, out_path: Path,
                     text_corrections: dict[str, str] | None = None) -> None:
    """Build an output-timeline SRT from per-source transcripts.

    - Chunks broken on natural sentence/pause boundaries (。！？!?））
    - Secondary break on commas (，,；;) for longer sentences
    - Natural case (no UPPERCASE conversion for Chinese)
    - Output times computed as word.start - segment_start + segment_offset
    - Optional text_corrections dict applied before rendering
    """
    transcripts_dir = edit_dir / "transcripts"
    sources = edl["sources"]

    # Sentence-ending punctuation — always break here
    SENTENCE_PUNCT = set("。！？!?）)")
    # Pause punctuation — break here if sentence is getting long
    PAUSE_PUNCT = set("，,；;")
    ...
    for r in edl["ranges"]:
        ...
        # Build phrases by breaking on natural pauses
        ...
        for w in words_in_seg:
            ...
            current.append(w)
            last_char = text[-1]
            char_count = sum(len((x.get("text") or "").strip()) for x in current)

            if last_char in SENTENCE_PUNCT:
                # Always break on sentence boundary
                phrases.append(current)
                current = []
            elif last_char in PAUSE_PUNCT and char_count >= 6:
                # Break on comma/pause if phrase is already 6+ chars
                phrases.append(current)
                current = []
            elif char_count >= 15:
                # Safety cap: break at 15 chars even without punctuation
                phrases.append(current)
                current = []
        ...
        # Merge very short consecutive phrases (under 4 chars) for readability
        merged: list[list[dict]] = []
        for phrase in phrases:
            text = "".join((w.get("text") or "").strip() for w in phrase)
...
def build_final_composite(
    ...
    if has_subs:
        ...
        font_dir = str(subtitles_path.resolve().parent.parent.parent / "static" / "fonts")
        if not Path(font_dir).exists():
            # Fallback: use SKILL_DIR/../static/fonts
            font_dir = str(edit_dir.parent / "static" / "fonts")
        font_opts = f"fontsdir='{font_dir}':force_style='{SUB_FORCE_STYLE}'"
        # Escape commas in force_style for filter_complex context
        font_opts = font_opts.replace(",", r"\,")
        filter_parts.append(
            f"{current}subtitles='{subs_abs}':{font_opts}[outv]"
        )
        out_label = "[outv]"
    else:
    ...
    # Use ffmpeg with libass support for subtitle compositing
